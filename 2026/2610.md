...
> $\mathrm{softmax}\left(\frac{QK^T}{\sqrt{d_k}}\right)$ 的结果是一个 $L \times L$ 的注意力权重矩阵，再乘以 $V$ 后得到 $L \times d_v$ 的输出。现代化LLM有些支持1M上下文，不可能真的 1M \* 1M 这么夸张的计算量，而是通过线性注意力、FlashAttention等方式优化，这个后面再看。这样可以把计算量从 $O(L^2)$ 压到接近 $O(Lw)$，其中 $w$ 是窗口大小。
...
虽然官方没有给确切的参数文档，但是我们能从网上搜索别人的评测估算或者拿公开 MoE 模型做参照：DeepSeek-V4-Pro 是 1.6T / 49B activated，Kimi-K3 是 2.8T / 104B activated，GLM-5 报告口径是 744B / 40B active。Google 早年的 GLaM 论文也公开过 1.2T 参数的稀疏激活 MoE 模型<sup><a href="https://arxiv.org/abs/2112.06905">21</a></sup>。这些模型给了一个大致范围：前沿 MoE 的总参数可以上 T，单 token 激活参数通常是几十 B 到一两百 B 这个量级。
...
| Kimi-K3         | 2.8T / 104B activated，视觉编码器 401M <sup><a href="https://huggingface.co/moonshotai/Kimi-K3">24</a></sup>     | 163,840（模型信息写 160K）<sup><a href="https://huggingface.co/moonshotai/Kimi-K3/blob/main/config.json">25</a></sup>        | 7,168    | 93，其中 1 层 dense   | 96 heads，`q_lora_rank=1536`，`kv_lora_rank=512`，69 层 KDA + 24 层 Gated MLA | 896 routed experts，16 selected/token，2 shared experts，Latent MoE dim 3584，`moe_intermediate_size=3072` | 1,048,576 |
...
| Kimi-K3         | $2 \times 163840 \times 7168 \approx 2.35B$ | $3 \times 3584 \times 3072 \approx 33.0M$ | $92 \times 896 \times 33.0M + 92 \times 2 \times 66.1M \approx 2.73T$ | $92 \times 16 \times 33.0M + 92 \times 2 \times 66.1M \approx 60.8B$ | 模型信息写 2.8T / 104B。这里 routed expert 按 Latent MoE 维度 3584 粗算，shared expert 按 hidden_size 7168 粗算；差额主要来自 69 层 KDA、24 层 Gated MLA 的注意力参数、路由器、Norm 和 401M 视觉编码器。 |
